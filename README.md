...
# バッチサイズ指定（デフォルト20）
uv run gmail-triage --batch 10
```

### 対象範囲

対象は `in:inbox -is:starred`、つまり**受信トレイにあってスターが付いていないメール**。既読・未読は区別しない。
スターを付けたものは常に対象外。アーカイブ済み・送信済み・迷惑メール・ゴミ箱も対象外。

`--hours` を付けると直近 N 時間に絞る。省略すれば期間で絞らない。

取得は ID 一覧を先に引いてから、バッチ単位（デフォルト20件）でメタデータ取得→分類→アクション実行を繰り返す。バッチごとに Discord 通知が送信され、複数バッチになった場合は合計サマリーも通知される。
